...
// 4) Generate next transaction_id, e.g. GM-0000001
...
if ($res && $res->num_rows === 1) {
    ...
    $num     = intval(substr($lastTid, 3)) + 1;  // strip "GM-"
} else {
    $num = 1;
}
...
$insert = $conn->prepare("
  INSERT INTO good_moral_requests
    (account_id, transaction_id, full_name, age, civil_status, purok, claim_date, purpose, payment_method)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
");
$insert->bind_param(
    "ississsss",
...
// Default: user panel for Good Moral
header("Location: ../userPanel.php?page=serviceGoodMoral&tid={$transactionId}");
exit();
?>
