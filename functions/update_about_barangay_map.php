...
if (!empty($_POST['description'])) {
    ...
    $stmt = $conn->prepare("UPDATE about_barangay_map SET description = ? WHERE id = ?");
...
if (!empty($_FILES['image']) && is_uploaded_file($_FILES['image']['tmp_name'])) {
    $stmt = $conn->prepare("SELECT image FROM about_barangay_map WHERE id = ?");
    ...
    $stmt = $conn->prepare("UPDATE about_barangay_map SET image = ? WHERE id = ?");
