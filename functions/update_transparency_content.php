...
if (!empty($_POST['description'])) {
    ...
    $stmt = $conn->prepare("UPDATE transparency_content SET description = ? WHERE id = ?");
...
if (!empty($_FILES['image']) && is_uploaded_file($_FILES['image']['tmp_name'])) {
    $stmt = $conn->prepare("SELECT image FROM transparency_content WHERE id = ?");
    ...
    $stmt = $conn->prepare("UPDATE transparency_content SET image = ? WHERE id = ?");
