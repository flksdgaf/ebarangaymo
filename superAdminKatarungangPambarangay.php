    ...
    <div class="d-flex align-items-center mb-3">
      ...
      <div class="dropdown">
        ...
        <div class="dropdown-menu p-3" aria-labelledby="filterDropdown" style="min-width:260px; --bs-body-font-size:.75rem; font-size:.75rem;">
          <form method="get" action="?page=superAdminComplaints" id="katarunganfilterForm" class="mb-0">
            <!-- preserve search -->
            ...
            <div class="d-flex">
              <a href="?page=superAdminComplaints&katarungan_page=1" class="btn btn-sm btn-outline-secondary me-2">Reset</a>
      ...
      <!-- Search form -->
      <form method="get" action="?page=superAdminComplaints" id="searchFormKatarungan" class="d-flex ms-auto me-2">
        <input type="hidden" name="page" value="superAdminComplaints">
        <input type="hidden" name="katarungan_date_from" value="<?= htmlspecialchars($date_from) ?>">
        <input type="hidden" name="katarungan_date_to" value="<?= htmlspecialchars($date_to) ?>">
        <input type="hidden" name="katarungan_page" value="1">
            ...
            ?>
              <tr data-id="<?= $tid ?>">
                ...
                <td class="text-center">
                  <!-- Edit -->
                  <a href="edit_katarungangpambarangay.php?transaction_id=<?= $tid ?>" class="btn btn-sm btn-success" title="Edit <?= $tid ?>">               
                    <span class="material-symbols-outlined" style="font-size:12px;">edit</span>
                  </a>

                  <!-- Delete -->
                  <button class="btn btn-sm btn-danger" title="Delete <?= $tid ?>" onclick="if (confirm('Delete complaint <?= $tid ?>? This cannot be undone.')) window.location='delete_katarungang.php?transaction_id=<?= $tid ?>';}">
