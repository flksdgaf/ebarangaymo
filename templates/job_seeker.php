...
if ($download || $print) {
    ...
    <body<?php if (!$includeHeader): ?> style="padding-top: 150px;"<?php endif; ?>>
      ...
      <div class="content">
        <p class="certification-title">CERTIFICATION FOR FIRST TIME JOB SEEKER</p>

        <p>
          This is to certify that
          <strong><?= htmlspecialchars(strtoupper($fullNameFormatted)) ?></strong>,
          <strong><?= htmlspecialchars($age) ?></strong> years old,
          <strong><?= htmlspecialchars(strtoupper($civilStatus)) ?></strong>, a bonafide resident of
          <?= htmlspecialchars($purok) ?> Magang Daet, Camarines Norte.
        </p>

        <p>
          This certifies further that the said person was informed of
          <?= strtolower($sex) === 'male' ? 'his' : (strtolower($sex) === 'female' ? 'her' : 'his/her') ?>
          rights, including the duties and responsibilities accorded by
          <strong>RA 11261</strong> through the <strong>Oath of Undertaking</strong>
          <?= strtolower($sex) === 'male' ? 'he' : (strtolower($sex) === 'female' ? 'she' : 'he/she') ?>
          has signed and executed in the presence of our Barangay Official.
        </p>

        <p class="no-indent">
          Signed this <strong><?= formatWithSuffix($issuedDate) ?></strong> day of
          <?= date('F, Y', strtotime($issuedDate)) ?> at the Barangay Magang, Daet, Camarines Norte.
        </p>

        <p style="margin-top: 40px; margin-bottom: 40px;">
          This certification is valid only one (1) year upon issuance.
        </p>
      </div>
    ...
    $dompdf->render();
    $filename = 'first_time_job_seeker_' . $transactionId . '.pdf';
...
<head>
<meta charset="UTF-8">
<title>First Time Job Seeker Certificate Preview</title>
  <style>
    body {
    ...
    .no-indent{ text-indent:0; font-size:14pt; }
    .signatory {
      margin-top: 60px;
      text-align: right;
      padding-right: 50px;
